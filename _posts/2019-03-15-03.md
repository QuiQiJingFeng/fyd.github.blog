---
layout: post
title:  "导出excel中的数据为lua表"
image: ''
date:   2019-03-15 14:24:31
tags:
- 工具
description: ''
categories:
...
for root, dirs, files in os.walk(targetDir + "excels", topdown=True):
    for f in fnmatch.filter(files, "*.xlsx"):
        ...
        for rownum in range(2, sh.nrows):
            ...
            for colnum in range(0, len(rowvalue)):
                #如果该值是浮点数，那么将其转换成整数
                value = sh.cell_value(rownum,colnum)
                ctype = sh.cell_type(rownum,colnum)
                ##空0 #字符串1 #数字2 #日期3 #布尔4
                if ctype == 0:
                    value = ""
                elif ctype == 1:
                    value = value
                elif ctype == 2 and value%1==0:        
                    value = int(value)
                elif ctype == 3:        
                    date_value = xlrd.xldate_as_tuple(value,0)
                    value = datetime(*date_value).strftime('%Y/%m/%d %H:%M:%S')
                elif ctype == 4:        
                    value = True if value == 1 else False
                elif ctype == 5:
                    value = error

                #将值读取并存入single 并以标题栏的字段作为KEY
                single[title[colnum]] = value
            #将单行的数据添加到数组当中
            convert_list.append(single)

        content = Util.ConvertJsonObjectToLuaStr(convert_list)
        fileName = os.path.splitext(f)[0]
        out_path = targetDir + fileName +".lua"
...
class Util:
    ...
    def generalProjectFile(dirs=['src', 'res'], extra=['*.git', '*.svn'], filts=['*.png', '*.json', '*.atlas', '*.csb', '*.lua', '*.plist', '*.pb'], path='project.json'):  
        ...
        content = Util.getStringFromFile('temp.md5')
        arrays = re.split('\n| ', content)
        size = len(arrays)
        srcMap = {}
        index = 0
            ...
            for root, dirs, files in os.walk(rootDir, topdown=True):
                ...
                for pat in includes:
                    for f in fnmatch.filter(files, pat):
                        suffix = Util.getLastSuffixName(f)
                        mode = 'rU'
                        if suffix in rblist:
                            mode = 'rb'
    ...
    def Zip(root_dir, out_file_name, includes,excludes=[]):
        ...
        for root, dirs, files in os.walk(root_dir, topdown=True):
            ...
            for pat in includes:
                for f in fnmatch.filter(files, pat):
                    file_path = os.path.join(root, f)
                    s = Util.getStringFromFile(file_path,'rb')
