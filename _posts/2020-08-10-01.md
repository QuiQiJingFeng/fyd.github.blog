...
![图片](..\assets\img\opengl\camera_axes.png)
...
2. 摄像机方向
下一个需要的向量是摄像机的方向，这里指的是摄像机指向哪个方向。现在我们让摄像机指向场景原点：(0, 0, 0)。还记得如果将两个矢量相减，我们就能得到这两个矢量的差吗？  
...
由于我们知道摄像机指向z轴负方向，但我们希望方向向量(Direction Vector)指向摄像机的z轴正方向。如果我们交换相减的顺序，我们就会获得一个指向摄像机正z轴方向的向量：
	...
	```
3. 我们知道了摄像机坐标系的Z轴,现在我们需要确定摄像机的X轴和Y轴。
   * 根据摄像机坐标系的Z轴方向,我们可以知道Z轴和Y轴所组成的平面内一定包含 世界坐标系中的Y轴，这个平面也可以用世界坐标系的Y轴 和摄像机坐标系的Z轴所表示，通过求解这个平面的垂直向量就可以求出X轴,有了X轴又可以求出Y轴。
   * 先定义一个上向量(世界坐标系Y轴正方向的单位向量),然后把上向量和摄像机Z轴方向的的向量进行叉乘,会得出指向X轴正方向的向量，(如果交换叉乘顺序会得到指向X轴负方向的向量)
   ```c
   glm::vec3 up = glm::vec3(0.0f, 1.0f, 0.0f); 
   glm::vec3 cameraRight = glm::normalize(glm::cross(up, cameraDirection));
   ```
4. 知道了X轴正方向向量、Z轴正方向向量,那么将X轴方向向量和Z轴方向向量进行叉乘可以得到Y轴方向的向量。
	```c
...
![图片](..\assets\img\opengl\look_at.png)
...
![图片](..\assets\img\opengl\camera_pitch_yaw_roll.png)
...
![图片](..\assets\img\opengl\camera_triangle.png)  
...
![图片](..\assets\img\opengl\camera_pitch.png)  
...
#### 看看我们是否能够为偏航角找到需要的分量：
![图片](..\assets\img\opengl\camera_yaw.png)  
...
```
注意，使用欧拉角的摄像机系统并不完美。根据你的视角限制或者是配置，你仍然可能引入万向节死锁问题。最好的摄像机系统是使用四元数(Quaternions)的，但我们将会把这个留到后面讨论。  

（译注：这里可以查看[四元数](https://github.com/cybercser/OpenGL_3_3_Tutorial_Translation/blob/master/Tutorial%2017%20Rotations.md)摄像机的实现）
...
```c
	...
	cameraPos.y = prePosY;
```
当我们向着相机朝向移动的时候,将Y轴的分量值的影响去掉就好了
